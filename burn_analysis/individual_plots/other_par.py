...
def _place_voids_in_grain_fast(grain, target_vol, rng_seed, img_size=1):
    """
    Place voids inside a single 3D grain up to target_vol.
    The caller is responsible for ensuring target_vol respects the per-grain cap.
    """
    ...
    for _ in range(800):
        ...
        rho_buf    = (pr - pore_r) * rho_buf ** (1/3)   # same formula as original
        ...
        # --- all logic below is identical to the original ---
...
def gen_struct_combined_2or3D(
    ...
    # ------------------------------------------------------------------
    # 2-D void placement
    # FIX-P: cap removed from here; caller passes a pre-capped budget.
    # ------------------------------------------------------------------
    ...
    if dim == 2:
        ...
        # PERF-3: return a list directly instead of yielding one item at a time
        ...
        # PERF-4: neighbors already a list; convert to numpy array once here
        ...
        for _ in range(max_attempts):
            ...
            for _ in range(100):
                ...
                nb = nearby_2d(x, y, circles, r)          # PERF-3: no list() wrapper needed
        ...
        # ---- Hollow grains — FIX-H: pre-sorted radii + increased max_pos ----
        ...
        hollow_radii_list = generate_radii(vol_percent_hollow * 1.8, mu_ln_h, sigma_ln_h, dim=3)
        ...
        for r in hollow_radii_list:
            ...
            for _ in range(max_pos):
                ...
                if not overlaps_fast_3d(x, y, z, r, nb, 0.98):
                    ...
                    spheres.append((x,y,z,r)); voids.append((x,y,z,rv))
                    grid[ci][cj][ck].append(len(spheres)-1)
                    hollow_vol += 4/3*math.pi*r**3 if fully_inside(x,y,z,r) else clipped_sphere_volume(x,y,z,r)
                    current_void_vol += 4/3*math.pi*rv**3 if fully_inside(x,y,z,rv) else clipped_sphere_volume(x,y,z,rv)
                    if len(spheres) % 1000 == 0:
                        print(f"  Hollow grains placed: {len(spheres)}  volume fraction: {hollow_vol:.4f}")
                    break
        ...
        porous_radii_list = generate_radii(vol_percent_porous*1.8, mu_ln_p, sigma_ln_p, dim=2)
        ...
        for r in porous_radii_list:
            ...
            for _ in range(max_pos):
                ...
                cx_c, cy_c = cell_coords(x, y)
                if len(grid[cx_c][cy_c]) > 15: continue
                nb = nearby_2d(x, y, circles, r)          # PERF-3
        ...
        if mwd_tolerance is not None:
            ...
            # PERF-6: pass radii directly, skip file re-read
        ...
        if vol_percent_porous > 0 and len(porous_circles) > 0:
            ...
            # FIX-O/Q: authoritative per-grain cap array; tracked cumulatively
        ...
        if vol_percent_porous > 0 and len(porous_circles) > 0:
            ...
            if over:
                ...
                print(f"  All grains within {MAX_VOID_FRACTION_PER_GRAIN*100:.0f}% cap. ✓")
    ...
    # ------------------------------------------------------------------
        ...
        for _ in range(max_attempts):
            ...
            for _ in range(100):
                ...
                ci, cj, ck = cell_coords(x, y, z)
                if len(grid[ci][cj][ck]) > 20: continue
                nb = nearby_3d(x, y, z, spheres, r)       # PERF-3: no list() wrapper needed
                if not overlaps_fast_3d(x, y, z, r, nb, 0.98):
        ...
        for _ in range(max_attempts):
            ...
            r = rng.lognormal(mu_ln, sigma_ln)
            if hollow_vol + 4/3*math.pi*r**3/total_domain_vol > vol_percent_hollow: continue
            ...
            for _ in range(max_pos):
                ...
                ci, cj, ck = cell_coords(x, y, z)
                if len(grid[ci][cj][ck]) > 20: continue
                nb = nearby_3d(x, y, z, spheres, r)       # PERF-3
                if not overlaps_fast_3d(x, y, z, r, nb, 0.98):
                    rv = r * (void_fraction / vol_percent_hollow) ** (1/3)
                    spheres.append((x,y,z,r)); voids.append((x,y,z,rv))
                    grid[ci][cj][ck].append(len(spheres)-1)
        ...
        sigma_ln_p = math.sqrt(math.log(1 + rad_dev**2))
        mu_ln_p    = math.log(img_r(mean_rad_porous)) - 1.5*sigma_ln_p**2
        porous_radii_list = generate_radii(vol_percent_porous*1.8, mu_ln_p, sigma_ln_p, dim=3)
        print(f"  Pre-generated {len(porous_radii_list)} candidate porous radii")
        ...
        # ---- Void placement — PERF-2: parallelized with ProcessPoolExecutor ----
        ...
        if vol_percent_porous > 0 and len(porous_spheres) > 0:
            ...
            # FIX-O/Q: authoritative per-grain cap; tracked cumulatively
            ...
            # PERF-2: submit all grains at once, collect results as they complete
            # — identical logic to the original sequential loop, just parallelized
            futures_map = {}
            with concurrent.futures.ProcessPoolExecutor() as executor:
                ...
                for fut in concurrent.futures.as_completed(futures_map):
                    ...
                    per_grain_placed[grain_idx] += vol_added   # FIX-O: cumulative tracking
                    # FIX-R: mark exhausted when cap reached
                    if per_grain_placed[grain_idx] >= per_grain_caps[grain_idx]:
                        exhausted_grains.add(grain_idx)
                    elif vol_added == 0:
                        exhausted_grains.add(grain_idx)
            ...
            # Top-up: redistribute remaining budget — logic identical to original
            for _ in range(20):
                ...
                for k, grain_idx in enumerate(active):
                    if current_void_vol >= target_void_vol: break
                    # FIX-Q: budget = min(proportional share, remaining cap for this grain)
                    remaining_cap = per_grain_caps[grain_idx] - per_grain_placed[grain_idx]
                    budget = min(redistrib[k], remaining_cap)
                    if budget <= 0:
                        exhausted_grains.add(grain_idx)
                    ...
                    if vol_added > 0:
                        progress = True
                    # FIX-R: mark exhausted when cap reached or placement fails
                    if per_grain_placed[grain_idx] >= per_grain_caps[grain_idx]:
                        exhausted_grains.add(grain_idx)
                    elif vol_added == 0:
                        exhausted_grains.add(grain_idx)
...
def _run_one_attempt(args):
    ...
    try:
        # PERF-6: radii already in memory inside gen_struct — but _run_one_attempt
        # only has the file path, so we read once here as before.
...
def generate_structures_with_target_mwd(
    ...
    print(f"Max void/grain: {MAX_VOID_FRACTION_PER_GRAIN*100:.0f}%")
    print(f"{'='*60}\n")
    ...
    common = dict(
        ...
        vol_percent_solid=vol_percent_solid, vol_percent_hollow=vol_percent_hollow,
        vol_percent_porous=vol_percent_porous, void_fraction=void_fraction,
        mwd_target=target_mwd, mwd_tolerance=mwd_tolerance, dim=dim,
        mean_rad_solid=mean_rad_solid, mean_rad_hollow=mean_rad_hollow,
        ...
        while pending and len(accepted) < n_target and attempt_idx <= max_total_attempts:
            ...
            for fut in done:
                ...
                if result is not None:
                    ...
                    result.update({"name": final_name, "ap_xyzr": final_ap, "void_xyzr": final_void})
                    accepted.append(result)
                    print(f"  → Kept as {final_name}  (accepted {len(accepted)}/{n_target})")
                    if accepted_idx == 0:
                        os.makedirs(subsubfolder, exist_ok=True)
                        plot_from_xyzr(final_ap, final_void,
    ...
    print(f"\nDone. {len(accepted)}/{n_target} structures accepted in {attempt_idx} total attempts.")
    return accepted
...
def main():
    generate_structures_with_target_mwd(
        ...
        dim=3,
        physical_size=50e-6,
        mean_rad_porous = 1.9e-6 / (1.2 * math.exp(math.sqrt(math.log(1 + 0.4 ** 2)) ** 2)), #target mwd
        mean_rad_hollow = 1.2e-6 / (1.2 * math.exp(math.sqrt(math.log(1 + 0.4 ** 2)) ** 2)),
        mean_rad_solid=2e-6,
        void_fraction=0.0,
        vol_percent_porous=0.55084,
